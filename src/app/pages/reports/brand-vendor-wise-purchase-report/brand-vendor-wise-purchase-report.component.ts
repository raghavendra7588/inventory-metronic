...
@Component({
  ...
  templateUrl: './brand-vendor-wise-purchase-report.component.html',
  styleUrls: ['./brand-vendor-wise-purchase-report.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class BrandVendorWisePurchaseReportComponent implements OnInit {
  ...
  viewPurchaseReport(response) {
    ...
    this.dialog.open(DialogBrandVendorWisePurchaseOrderComponent, {
      height: '650px',
      width: '1500px',
      data: response
    });
  ...
  searchRecords() {
    ...
    this.spinner.show();
    this.reportsService.postBrandVendorWisePurchaseReport(this.purchaseReport).subscribe(data => {

      this.purchaseReportResponse = data;

      let uniquePurchaseOrder = _.uniqBy(this.purchaseReportResponse, 'BrandID');
